// Standalone WhatsApp Channel Example
//
// Minimal example: start a single WhatsApp bot using channel.New().
// All message handling, streaming replies, and session management
// are handled by the channel package — no boilerplate needed.
//
// First run prints a QR code in the terminal for pairing.
//
// Usage:
//
...
func main() {
	...
	ch.Start(ctx)
	logger.Info("standalone whatsapp channel started")
